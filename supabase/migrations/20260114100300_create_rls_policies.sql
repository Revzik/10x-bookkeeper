...
-- Rationale: Users can edit note content and embedding status
...
-- Policy: Allow authenticated users to SELECT their own note embeddings
-- Rationale: Users should only access embeddings generated from their notes
...
-- Policy: Allow authenticated users to INSERT their own note embeddings
-- Rationale: Background jobs create embeddings for user's notes
...
-- Policy: Allow authenticated users to UPDATE their own note embeddings
-- Rationale: Embeddings may need to be regenerated if content changes
...
-- Policy: Allow authenticated users to DELETE their own note embeddings
-- Rationale: Embeddings should be removed when notes are deleted or regenerated
...
-- Policy: Allow authenticated users to SELECT their own reading sessions
-- Rationale: Users should only see their own reading history
...
-- Policy: Allow authenticated users to INSERT their own reading sessions
-- Rationale: Users create reading sessions when they start reading
...
-- Policy: Allow authenticated users to UPDATE their own reading sessions
-- Rationale: Sessions need to be updated when reading stops (ended_at, end_page)
...
-- Policy: Allow authenticated users to DELETE their own reading sessions
-- Rationale: Users may want to remove erroneous session entries
...
-- ============================================================
-- RLS Policies for: embedding_errors
-- ============================================================

-- Policy: Allow authenticated users to SELECT their own embedding errors
-- Rationale: Users and admin tools should see embedding failures for their notes
create policy "Authenticated users can select their own embedding errors"
on public.embedding_errors
for select
to authenticated
using (user_id = auth.uid());

-- Policy: Allow authenticated users to INSERT their own embedding errors
-- Rationale: Background processes log embedding failures for debugging
create policy "Authenticated users can insert their own embedding errors"
on public.embedding_errors
for insert
to authenticated
with check (user_id = auth.uid());

-- Policy: Allow authenticated users to UPDATE their own embedding errors
-- Rationale: Error records may need status updates or annotations
create policy "Authenticated users can update their own embedding errors"
on public.embedding_errors
for update
to authenticated
using (user_id = auth.uid())
...
-- Policy: Allow authenticated users to DELETE their own embedding errors
-- Rationale: Users may want to clear resolved errors
create policy "Authenticated users can delete their own embedding errors"
on public.embedding_errors
for delete
to authenticated
using (user_id = auth.uid());
