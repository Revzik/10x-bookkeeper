...
export const DeleteNoteDialog = ({ open, onOpenChange, noteId, chapterTitle, onDeleted }: DeleteNoteDialogProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      setGeneralError(result.error?.generalError || t("note.deleteError"));
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          ...
          <DialogDescription>{t("note.deleteDescription", { title: chapterTitle })}</DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="rounded-lg border border-destructive bg-destructive/10 p-3 text-sm text-destructive">
            <strong>{t("note.deleteWarningTitle")}</strong> {t("note.deleteWarningBody")}
          </div>
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            <Button type="submit" variant="destructive" disabled={isDeleting}>
              {isDeleting ? t("note.deleting") : t("note.deleteConfirm")}
            </Button>
          </div>
        </form>
      </DialogContent>
